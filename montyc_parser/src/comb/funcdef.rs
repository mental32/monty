...
use crate::comb::{expect_many_n_var, whitespace};
...
pub fn function_def<'this, 'source, 'data>(
    ...
    let (stream, (receiver, mut arguments)) = arguments(stream)?;
    ...
    let args = arguments
        ...
        .map(|(l, r)| match (l.inner, r) {
            (PyToken::IdentRef(named), annotation) => Spanned {
                span: l.span,
                inner: FunctionDefParam { named, annotation },
            },
            _ => unreachable!(),
        })
        .collect::<Vec<_>>();
    ...
    let name = match ident.inner {
        PyToken::IdentRef(n) => Atom::Name(n),
        _ => unreachable!(),
    };
